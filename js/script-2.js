$(document).on('submit','form.ajax',function(){return diafan_ajax.init(this);});$('.left-side').ready(function(){var insertValue=$(this).find('.previous_next_links').html();$('.bottom-navigation').html(insertValue);});var diafan_ajax={before:{},success:{},tag:false,init:function(form){this.tag=$("input[name=module]",form).val()+"_"+$("input[name=action]",form).val();if(this.before[this.tag]&&this.before[this.tag](form)===false)
{return false;}
